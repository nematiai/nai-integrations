...
class DropboxService(BaseCloudService):
    ...
    def _load_auth(self) -> None:
        try:
            self.auth = DropboxAuth.objects.get(
    ...
    def _get_credentials(self) -> tuple[str, str]:
        client_id = getattr(settings, "DROPBOX_CLIENT_ID", "") or getattr(
            settings, "DROPBOX_APP_KEY", ""
        )
        client_secret = getattr(settings, "DROPBOX_CLIENT_SECRET", "") or getattr(
    ...
    def list_folder_continue(self, cursor: str) -> Dict[str, Any]:
        response = self._make_api_request(
            "POST", "files/list_folder/continue", json={"cursor": cursor}
        )
        return response.json()
